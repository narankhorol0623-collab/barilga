  ...
  return (
    <>
      ...
      <main>
        <section className="relative isolate flex h-svh min-h-[680px] items-center overflow-hidden bg-[radial-gradient(circle_at_76%_34%,rgba(20,93,148,.22),transparent_31%),radial-gradient(circle_at_25%_72%,rgba(32,68,135,.1),transparent_28%),linear-gradient(125deg,#0a1128_0%,#0a1128_58%,#0d1732_100%)] px-[clamp(20px,7vw,120px)] pb-[60px] pt-[100px] max-[760px]:min-h-[760px] max-[760px]:px-4 max-[760px]:pb-20 in-data-[theme=light]:bg-[radial-gradient(circle_at_76%_34%,rgba(20,93,148,.13),transparent_31%),radial-gradient(circle_at_25%_72%,rgba(33,106,171,.1),transparent_28%),linear-gradient(125deg,#f7f9fc_0%,#eef3f9_58%,#e6edf6_100%)] after:absolute after:right-[-20%] after:top-[10%] after:h-[60vw] after:w-[60vw] after:rotate-[35deg] after:border after:border-[#a0b2da]/8 after:content-['']">
          ...
          <div className="relative z-[2] max-w-[850px] py-3">
            <div className="mb-7 inline-flex items-center gap-2.5 rounded-full border border-[var(--brand-accent)]/20 bg-[#101d3b]/70 px-4 py-2.5 text-xs font-extrabold tracking-[.18em] text-[color:var(--brand-accent)] shadow-[0_12px_38px_rgba(0,0,0,.16)] backdrop-blur-lg in-data-[theme=light]:bg-white/70 in-data-[theme=light]:text-[#216aab]"><i className="size-2 rounded-full bg-[var(--brand-accent)]" /> ШИНЭ ТӨСӨЛ НЭЭЛТТЭЙ</div>
            ...
            <div className="flex items-stretch gap-4 max-[760px]:flex-col">
              <Link className={primaryButtonClass} href="/master-plan">БАЙРАА СОНГОХ ↗</Link>
          ...
          <Image width={1597} height={985} className="absolute bottom-6 left-[clamp(20px,7vw,120px)] h-12 w-20 object-contain opacity-90 max-[760px]:bottom-[30px]" src="/gund-supply-logo.webp" alt="" />
          <a className="absolute bottom-6 left-1/2 z-[2] -translate-x-1/2 text-center text-[9px] tracking-[.25em] text-[#778294]" href="#about">SCROLL <b className="block text-[26px] text-[color:var(--brand-accent)]">⌄</b></a>
        </section>
          ...
          <div className="mx-auto max-w-[1240px]">
            <div className="grid grid-cols-[.8fr_1.2fr] gap-20 max-[900px]:grid-cols-1 max-[900px]:gap-8">
              <div>
                ...
                <h2 className="mt-3 text-[clamp(34px,4vw,56px)] leading-[1.08] tracking-[-.04em]">Туршлагаас<br /><span className="text-[color:var(--brand-accent)]">үнэ цэн бүтээнэ.</span></h2>
              </div>
            ...
            <div className="relative mt-16 grid grid-cols-4 gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 max-[820px]:grid-cols-2 max-[480px]:grid-cols-1 in-data-[theme=light]:border-[#ccd5e2] in-data-[theme=light]:bg-[#ccd5e2]">
              {milestones.map((item) => (
                <article className="bg-[#0d1730] p-7 in-data-[theme=light]:bg-white" key={item.year}>
                  <strong className="text-[32px] tracking-[-.04em] text-[color:var(--brand-accent)]">{item.year}</strong>
            ...
            <div className={`${glassClass} relative mt-6 overflow-hidden rounded-2xl p-10 max-[760px]:p-6`}>
              ...
              <div className="relative grid grid-cols-[auto_1fr] gap-10 max-[760px]:grid-cols-1 max-[760px]:gap-5">
                <div className="text-[56px] font-black leading-none text-[color:var(--brand-accent)]">3.8<span className="ml-1 text-lg">га</span></div>
                <div>
                  ...
                  <div className="mt-6 flex flex-wrap gap-3 text-xs font-extrabold tracking-[.08em]"><span className="rounded-full border border-[var(--brand-accent)]/30 px-4 py-2 text-[color:var(--brand-accent)]">5 БЛОК</span><span className="rounded-full border border-[var(--brand-accent)]/30 px-4 py-2 text-[color:var(--brand-accent)]">22 АМИНЫ ОРОН СУУЦ</span><span className="rounded-full border border-[var(--brand-accent)]/30 px-4 py-2 text-[color:var(--brand-accent)]">БҮРЭН ДУУССАН</span></div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6 max-[760px]:grid-cols-1">
              <article className={`${glassClass} rounded-2xl border-t-4 border-t-[var(--brand-accent)] p-8 max-[760px]:p-6`}>
            ...
            <div className="mt-20 max-[760px]:mt-14">
              ...
              <div className="grid grid-cols-3 gap-4 max-[900px]:grid-cols-2 max-[600px]:grid-cols-1">
                {values.map((value) => (
                  <article className={`${glassClass} group rounded-xl p-6 transition-colors hover:border-[var(--brand-accent)]/50`} key={value.no}>
                    <span className="text-xs font-extrabold tracking-[.16em] text-[color:var(--brand-accent)]">{value.no}</span>
                    <h3 className="mb-3 mt-6 text-xl font-bold group-hover:text-[color:var(--brand-accent)]">{value.title}</h3>
          ...
          <div className="grid grid-cols-2 gap-6 max-[760px]:grid-cols-1">
            ...
            {projects.map((project) => (
              <article className="group relative h-[670px] overflow-hidden rounded-[14px] max-[760px]:h-[480px] after:absolute after:inset-x-0 after:bottom-0 after:top-[35%] after:bg-gradient-to-b after:from-transparent after:to-[#060c1d] after:content-['']" key={project.name}>
                ...
                <span className="absolute right-5 top-5 z-[3] rounded-full border border-[var(--brand-accent)]/50 bg-[#071a30]/70 px-3.5 py-[7px] text-[11px] font-extrabold text-[color:var(--brand-accent)]">{project.status}</span>
        ...
        <section className="px-[clamp(20px,7vw,120px)] py-[100px] text-center max-[760px]:px-4 max-[760px]:py-[72px]">
          <div className={`${glassClass} mx-auto max-w-[1020px] rounded-[20px] px-[50px] py-[70px] max-[760px]:px-5 max-[760px]:py-[46px]`}>
            <div className="text-[58px] text-[color:var(--brand-accent)]">♧</div>
      ...
      <footer id="contact" className="grid grid-cols-[1.2fr_1fr_1fr] items-center gap-10 border-t border-white/7 bg-[#060c1d] px-[clamp(20px,7vw,120px)] py-[52px] max-[760px]:grid-cols-1 max-[760px]:pb-[110px] max-[760px]:text-center in-data-[theme=light]:bg-[#e8eef6]">
        <div className="space-y-4"><BrandLogo /><p className="leading-[1.7] text-[#b7c0d2] in-data-[theme=light]:text-[#526078]">Өндөр зэрэглэлийн үл хөдлөх хөрөнгийн зах зээлд тэргүүлэгч.</p></div>
      ...
      <nav className={bottomNavClass}><Link className="!text-[color:var(--brand-accent)]" href="/">⌂<span>НҮҮР</span></Link><Link href="/master-plan">▥<span>ТӨСЛҮҮД</span></Link><a href="#">▱<span>ХАДГАЛСАН</span></a><a href="#contact">?<span>ХОЛБОО</span></a></nav>
    </>
  );
}
